...
import styles from './Recordmes.less';
...
// }))
@Form.create()
class Recordmes extends PureComponent {
  state = {
    modalVisible: false,
  ...
  render() {
    ...
    const columns = [{
      ...
      render:(val,record)=>(
        <div>
            <Badge status={ val == 2 ? 'processing' : val == 1 ? 'success' : val == 0 ? 'default' : val == -1 ? 'error' : 'error'} />{val === -1 ? '商家取消' : val === 1 ? '已接单' : val === 2 ? '已完成' : val === 0 ? '待接单' : null}
        </div>
    )
    },{
      width:150,
